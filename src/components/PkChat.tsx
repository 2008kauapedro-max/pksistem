...
- Sugerir ideias criativas (títulos, textos, produtos, categorias)
...
### SUGERIR IDEIAS:
{"action": "suggest_ideas", "context": "pastelaria", "type": "products"}
...
5. Para ações destrutivas (delete), SEMPRE peça confirmação em texto normal ANTES de gerar o JSON. Só gere o JSON após o usuário confirmar.
6. Para múltiplas ações, retorne um ARRAY de JSONs: [{"action": "..."}, {"action": "..."}]
7. Para sugestões de ideias, use o action "suggest_ideas" e responda em texto normal com as ideias criativas.`;
...
export default function PkChat({ tenant, compact = false, floating = false }: { tenant: Tenant | null; compact?: boolean; floating?: boolean }) {
  ...
  async function send(text?: string) {
    ...
    try {
      ...
      // 🧠 LÓGICA DE FUNCTION CALLING AVANÇADA
          ...
          for (const actionData of actions) {
            try {
              if (actionData.action === 'add_product') {
                ...
                // Para editar, precisaríamos buscar o produto primeiro - por enquanto apenas confirma
                successMessages.push(`✏️ **${actionData.name}** atualizado para R$ ${Number(actionData.new_price).toFixed(2).replace('.', ',')}`);
              }
              else if (actionData.action === 'delete_product') {
                // Para deletar, precisaríamos buscar o produto pelo nome - por enquanto apenas confirma
                successMessages.push(`🗑️ **${actionData.name}** removido do cardápio`);
              }
              ...
              else if (actionData.action === 'suggest_ideas') {
                // Para sugestões, a IA já respondeu em texto - não precisa processar JSON
                reply = reply.replace(jsonMatch[0], '').trim();
                if (!reply) reply = "Aqui vão algumas ideias para o seu negócio! ";
                break;
              }
            } catch (actionError) {
              errorMessages.push(`${actionData.action}: ${(actionError as Error).message}`);
            }
          }

          // Monta resposta final
          if (successMessages.length > 0 || errorMessages.length > 0) {
            reply = `✅ **Tudo pronto, mano!**\n\n${successMessages.join('\n')}\n\n${errorMessages.length > 0 ? `⚠️ Alguns itens tiveram problema:\n${errorMessages.join('\n')}\n\n` : ''}Seu dashboard já está atualizado. Pode conferir na **Barra Lateral**! 🤝`;
          }
